...
{
  bool result = false; /* false: DateTime update not done, true: DateTime update done */
  ...
  if (HAL_RTC_SetTime(&hrtc, &rtc_time, RTC_FORMAT_BIN) == HAL_OK)
  {
    ...
    {
      result = true; /* DateTime update done */
      cellular_app_datetime_initialized = true; /* DateTime is initialized */
      cellular_app_datetime_year_start = p_datetime->date.year_start;
      /* If a callback is registered, inform that DateTime has been updated */
...
{
  /* Only one regsitartion is possible at a time, the last caller is the winner */
