...
/* Process a EchoClt performance test iteration loop */
...
/* Process a EchoClt performance test */
static void echoclient_performance(cellular_app_socket_desc_t *const p_socket);
/* Check if EchoClt is blocked or not(e.g data not ready, performance in progress, process status inactive) */
static bool echoclient_is_blocked(uint8_t index);
/* Get for EchoClt application an unique index */
...
static void echoclient_analyze_datetime_str(uint8_t rcv_len, uint8_t const *p_rcv)
{
  ...
  /* datetime items order received in the string: Day, Month, Monthday, Time, Year */
  uint8_t distant_datetime_order_received[5] = {0, 2, 1, 4, 3};

  uint8_t i = 0U; /* index in the string to analyze */
  uint8_t j = 0U; /* 5 items must be found in a string: Day, MonthDay, Month, Year, Time */
  uint8_t offset; /* index in the datetime string datetime_str[] */
  uint8_t start = 0U; /* use to calculate the length of an item */
  uint8_t argv[5];    /* save the start index of each items     */
  uint8_t len[5];     /* save the length of each items          */
  uint8_t datetime_str[30]; /* save a datetime string in always same order: Day:0 MonthDay:1 Month:2 Year:3 Time:4 */
  bool next = true;   /* false: an item is in progress(its length is under calculation)
                       * true : no item in progress */
  ...
  while ((i < rcv_len) && (j < 5U))
  {
    ...
    {
      /* if an item is in progress a ' ' '\n' '\r' sign its end */
      if (next == false)
      {
        len[j] = i - start; /* item length calculation */
        j++; /* go to next item */
      }
      next = true; /* no item in progress */
    }
    else
    {
      /* if no item in progress a character != ' ' '\n' 'r' sign its begin */
      if (next == true)
      {
        argv[j] = i;  /* save the start index of the item */
        next = false; /* an item is in progress */
        start = i;    /* save begin of the item */
      }
    }
    i++; /* got to next character in the string */
  }
  ...
  if (j == 5U)
  {
    ...
    {
      /* Copy each item in the same order */
      (void)memcpy(&datetime_str[offset], &p_rcv[argv[distant_datetime_order_received[k]]],
                   len[distant_datetime_order_received[k]]);
      offset += len[distant_datetime_order_received[k]]; /* update offset with the copied bytes */
      datetime_str[offset] = (uint8_t)' '; /* add a ' ' between each item */
      offset++;
    }
    /* Replace last ' ' by '\0' */
    offset--;
    datetime_str[offset] = (uint8_t)'\0';
    /* Request datetime string conversion to a cellular_app_datetime_t item */
    if (cellular_app_datetime_str_convert(offset, (uint8_t const *)&datetime_str[0], &datetime) == true)
    {
      /* Request datetime update */
  ...
  {
    /* Analysis of the receveid string failed */
...
static void echoclient_obtain_datetime(cellular_app_socket_desc_t *p_socket)
{
  uint16_t distant_datetime_port; /* distant port where datetime information can be found */
  ...
  {
    int32_t ret; /* Send/Receive result */
    ...
    /* If distantip is known, send the request to obtain the datetime */
    if (CELLULAR_APP_GET_DISTANTIP(p_socket->distant.ip) != (uint32_t)0U)
    {
      bool connected = false; /* false: not connected to the distant, true: connected */
      ...
      if (p_socket->id > COM_SOCKET_INVALID_ID)
      {
        ...
        /* Request to connect to the distant */
      ...
      /* If connection ok, close the socket */
      if (p_socket->id > COM_SOCKET_INVALID_ID)
      {
        (void)com_closesocket(p_socket->id); /* Whatever the result, try only one time to close the socket */
      }
    }
  }
}
...
/**
  * @brief  Format the buffer to send by EchoClt
...
static uint16_t echoclient_format_buffer(uint16_t length, uint8_t *p_buffer)
{
  /* Add in the buffer number from 0 to 9 */
  uint8_t  number = 0U; /* first trame number is 0 */
  uint16_t i;  /* Index in the buffer [0, length[*/
  ...
  /* Copy datetime as first characters of the buffer */
  ...
  /* index is updated accordingly the datetime copied */
  i = (uint16_t)crs_strlen(&p_buffer[0]); /* Doing a sprintf("%02d:%02d:%02d - %04d/%02d/%02d") a '\0' is available */
  ...
  /* Pad the rest of buffer with ASCII number 0x30 0x31 ... 0x39 0x30 ...*/
  while (i < length)
  {
    /* Add next number in the buffer */
    p_buffer[i] = 0x30U + number; /* 0x30U : to have ASCII number */
    number++; /* go to next number */
    /* Is limit reached ? */
    if (number == 10U)
    {
      number = 0U; /* restart at 0 */
    }
    i++; /* go to next index */
  }
  /* Set last byte ogf the buffer to '\0' */
...
{
  bool result = false; /* false: process nok, true: process ok */
  int32_t read_buf_size;
  int32_t address_len;
  com_sockaddr_in_t address;
  uint32_t time_begin; /* used for performance: tick when send is done */
  uint32_t time_end;   /* used for performance: tick when receive is done */
  uint8_t *p_welcome_msg = NULL; /* some distant with some protocol send a first trame as a welocme */
      ...
      == true)
  {
        ...
        && (p_welcome_msg != NULL)) /* before to send anything a welcome msg will be received */
    {
      ...
      {
        /* Welcome msg is ok, update socket state according to the socket protocol */
        if ((p_socket->protocol == CELLULAR_APP_SOCKET_TCP_PROTO)
            || (p_socket->protocol == CELLULAR_APP_SOCKET_UDP_PROTO))
        {
      ...
      {
        /* Welcome msg is nok, socket will be closed */
    ...
    if ((p_socket->state == CELLULAR_APP_SOCKET_CONNECTED) || (p_socket->state == CELLULAR_APP_SOCKET_CREATED))
    {
          ...
          || (p_socket->protocol == CELLULAR_APP_SOCKET_UDP_PROTO))
      {
        ...
        time_begin = HAL_GetTick(); /* save the tick to then calculate the time of the exchange transaction */
      ...
      if (ret == (int32_t)p_socket->snd_buffer_len)
      {
        ...
        if ((p_socket->protocol == CELLULAR_APP_SOCKET_TCP_PROTO)
            || (p_socket->protocol == CELLULAR_APP_SOCKET_UDP_PROTO))
        {
          bool exit = false; /* false: continue to received data, true: stop the reception process */
          ...
          do
          {
            ...
            if (read_buf_size < 0)
            {
              exit = true; /* exit from the reception process */
            }
            else /* Some data received */
            {
              /* No error during the reception */
        ...
        /* Are all data send received ? */
        if ((int32_t)p_socket->snd_buffer_len == total_read_size)
        {
          /* Are all data send have been received */
          ...
          {
            /* Data received are nok, increase fault counters, request close socket */
      ...
      {
        /* Send data is nok, increase fault counters, request close socket  */
    ...
    else /* no socket available to do the exchange */
    {
...
static void echoclient_performance_iteration(cellular_app_socket_desc_t *const p_socket,
                                             ...
                                             echoclient_performance_result_t *p_perf_result)
{
  /* bool exit = false; */ /* false: continue the treatment, true: stop the treatment */
  uint16_t i = 0U; /* iteration counter [0, iteration_nb[ */
  uint32_t time_snd_rcv; /* time to do a send/receive iteration */
  ...
  if (p_socket->snd_buffer_len != 0U)
  {
    ...
    while (i < iteration_nb)
    {
      ...
      {
        /* Process ok, update iter ok and total time of transaction */
...
#define ECHOCLIENT_PERFORMANCE_TCP_TRAME_MAX ((uint16_t)(1400U)) /* trame max size in TCP (in bytes) */
#define ECHOCLIENT_PERFORMANCE_UDP_TRAME_MAX ((uint16_t)(1400U)) /* trame max size in UDP (in bytes) */
  uint16_t iter_total = 0U; /* total iteration (used for validation)    */
  uint16_t iter_ok = 0U;    /* total iteration ok (used for validation) */

  static uint16_t trame_size_in_TCP[ECHOCLIENT_PERFORMANCE_NB_ITER] =
  {
    16U, 32U, 64U, 128U, 256U, 512U, 1024U, ECHOCLIENT_PERFORMANCE_TCP_TRAME_MAX
  };
  static uint16_t trame_size_in_UDP[ECHOCLIENT_PERFORMANCE_NB_ITER] =
  {
    16U, 32U, 64U, 128U, 256U, 512U, 1024U, ECHOCLIENT_PERFORMANCE_UDP_TRAME_MAX
  };
  /* Default iteration number for each trame_size */
  static uint16_t iter[ECHOCLIENT_PERFORMANCE_NB_ITER] =
  {
    1000U, 1000U, 1000U, 1000U, 200U, 100U, 100U, 100U
  };
  /* Performance result table for each iteration */
  static echoclient_performance_result_t perf_result[ECHOCLIENT_PERFORMANCE_NB_ITER];
  uint16_t *p_trame_size; /* according to the protocol, point on trame_size_in_TCP or on trame_size_in_UDP */

  /* Update p_trame_size according to the socket protocol */
  if (p_socket->protocol == CELLULAR_APP_SOCKET_TCP_PROTO)
  {
  ...
  /* Performance execution for each iteration */
  for (uint8_t i = 0U; i < ECHOCLIENT_PERFORMANCE_NB_ITER; i++)
  {
    /* Use default iteration number: iter[] or a specific iteration number has been defined ? */
    if (cellular_app_echoclient_perf_iter_nb == 0U)
    {
      /* Use default iteration number: iter[] */
      echoclient_performance_iteration(p_socket, iter[i], p_trame_size[i], &perf_result[i]);
    }
    else
    {
      /* Use a specific iteration number */
  ...
  for (uint8_t i = 0U; i < ECHOCLIENT_PERFORMANCE_NB_ITER; i++)
  {
    /* Calculate the number of bytes correctly send and received */
    uint32_t data_snd_rcv = (uint32_t)(p_trame_size[i]) * 2U * (uint32_t)(perf_result[i].iter_ok);
    /* Number of iteration done for each loop ? */
    if (cellular_app_echoclient_perf_iter_nb == 0U)
    {
...
static bool echoclient_is_blocked(uint8_t index)
{
  bool result = false; /* false: process not blocked, true: process blocked */
      ...
      || ((index == 0U)
          && (((cellular_app_echoclient_perf_start == true)
              ...
              || ((cellular_app_echoclient_perf_start == false)
                  && (cellular_app_echoclient_change[index].process_status == false)))))
  {
    result = true; /* process is blocked */
  }
...
/**
  * @brief  Get for EchoClt application an unique index
  * @param  -
...
static void echoclient_thread(void *p_argument)
{
  ...
  /* Obtain an unique EchClt index */
  app_index = echoclient_get_app_index();
  /* Is provided index ok ? */
  if (app_index < ECHOCLIENT_THREAD_NUMBER)
  {
    ...
    {
      cellular_app_datetime_t datetime;
      /* DateTime maybe already initialized by another process ? */
      if (cellular_app_datetime_get(&datetime) == true)
      {
        /* DateTime already initialized by another process */
      ...
      while (cellular_app_echoclient_set_datetime == true)
      {
               ...
               || (cellular_app_echoclient_change[app_index].process_status == false))
        {
          ...
          /* Wait process is ok and data is ready */
    ...
    for (;;)
    {
      ...
      {
        PRINT_INFO("%s %d: Process stopped or Data not ready! Closing the socket!", p_cellular_app_echoclient_trace,
                   (app_index + 1U))
        ...
        do
        {
          if (cellular_app_is_data_ready() == true)
          {
            PRINT_INFO("%s %d: Closing the socket", p_cellular_app_echoclient_trace, (app_index + 1U))
...
{
  bool result = true; /* false: message not send, true: message send */
  uint8_t i = 0U;
  uint8_t index_limit;
  ...
  while (i < index_limit)
  {
    ...
    if (status != osOK)
    {
      cellular_app_msg_type_t msg_type = GET_CELLULAR_APP_MSG_TYPE(queue_msg);
      cellular_app_msg_id_t   msg_id   = GET_CELLULAR_APP_MSG_ID(queue_msg);

      /* Message send failed */
      result = false;
      PRINT_FORCE("%s: ERROR CellularApi Msg Put App:%d Type:%d Id:%d - status:%d!", p_cellular_app_echoclient_trace,
                  (i + 1U), msg_type, msg_id, status)
    }
    i++; /* Send message to next EchoClt */
  }
...
{
  cellular_app_process_status_t result = CELLULAR_APP_PROCESS_STATUS_MAX; /* index is unknown */
...
{
  bool result = false; /* false: message not send, true: message send */
  ...
  if (index < ECHOCLIENT_THREAD_NUMBER)
  {
    process_status_tmp = cellular_app_get_status(cellular_app_echoclient[index].process_status,
                                                 cellular_app_echoclient_change[index].process_status);
    /* Only one process change at a time */
    ...
    {
      cellular_app_msg_type_t msg_type = CELLULAR_APP_PROCESS_MSG;
      cellular_app_msg_id_t   msg_id   = CELLULAR_APP_PROCESS_CHANGE_ID;
      uint32_t queue_msg = 0U;

      SET_CELLULAR_APP_MSG_TYPE(queue_msg, msg_type);
      SET_CELLULAR_APP_MSG_ID(queue_msg, msg_id);
      ...
      if (status != osOK)
      {
        /* Message send failed, restore old process status value */
        cellular_app_echoclient_change[index].process_status = !process_status;
        PRINT_FORCE("%s ERROR SetStatus Msg Put App:%d Type:%d Id:%d - status:%d!", p_cellular_app_echoclient_trace,
                    (index + 1U), msg_type, msg_id, status)
      }
      else
      {
        result = true; /* message send */
      }
    }
    else
    {
...
{
  bool result = false; /* false: process period change not requested, true: process period change requested */
  ...
  if ((index < ECHOCLIENT_THREAD_NUMBER) && (process_period > ECHOCLIENT_PROCESS_PERIOD_MIN))
  {
    ...
    result = true; /* process period change requested */
  }
...
{
  bool result = false; /* false: buffer length change not requested, true: buffer length change requested */
  ...
  if (index < ECHOCLIENT_THREAD_NUMBER)
  {
    if ((snd_buffer_len >= ECHOCLIENT_SND_RCV_MIN_SIZE) && (snd_buffer_len <= ECHOCLIENT_SND_RCV_MAX_SIZE))
    {
      ...
      result = true; /* buffer length change requested */
    }
    else
    {
...
{
  bool result = false; /* false: protocol change not requested, true: protocol change requested */
  ...
  if (index < ECHOCLIENT_THREAD_NUMBER)
  {
    ...
    else
    {
      ...
      {
        result = true; /* protocol change requested */
        ...
        /* Is protocol change immediately possible ? */
...
{
  bool result = false; /* false: distant change not requested, true: distant change requested */
...
{
  bool result = false; /* false: socket statistics not found, true: socket statistics found */

  if (index < ECHOCLIENT_THREAD_NUMBER)
  {
    /* Copy the socket statistics */
    (void)memcpy((void *)p_stat, (const void *) & (cellular_app_echoclient_socket[index].stat),
                 sizeof(cellular_app_socket_stat_desc_t));
    result = true; /* socket statistics found */
  }
...
{
  bool result = true; /* false: performance not requested, true: performance requested */

  /* Performonance start requested ? */
  if (status == true)
  {
    /* Check EchoClt instance 0 is available */
#if (USE_RTC == 1)
  ...
  else /* Performonance start canceled */
  {
    cellular_app_echoclient_perf_start = false;
    cellular_app_echoclient_perf_iter_nb = iter_nb;
  }
...
void cellular_app_echoclient_display_status(void)
{
  ...
  for (uint8_t i = 0U; i < ECHOCLIENT_THREAD_NUMBER; i++)
  {
    ...
    {
      PRINT_FORCE("Process period change in progress New value:%ldms", cellular_app_echoclient_change[i].process_period)
    }
...
void cellular_app_echoclient_init(void)
{
  ...
  cellular_app_echoclient_index_mutex_handle = rtosalMutexNew((const rtosal_char_t *)"CELLAPPIOT_MUT_ECHO_IND");
  /* Is Mutex creation ok ? */
  ...
  for (uint8_t i = 0U; i < ECHOCLIENT_THREAD_NUMBER; i++)
  {
    ...
    cellular_app_echoclient[i].queue_id = rtosalMessageQueueNew((const rtosal_char_t *)"CELLAPPIOT_QUE_ECHO_SOCKET",
