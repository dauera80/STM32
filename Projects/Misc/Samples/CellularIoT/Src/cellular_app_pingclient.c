...
static void pingclient_thread(void *p_argument)
{
  ...
  for (;;)
  {
    ...
    while ((cellular_app_is_data_ready() == false) || (cellular_app_pingclient_change.process_status == false))
    {
...
bool cellular_app_pingclient_set_status(bool process_status)
{
  ...
  {
    PRINT_FORCE("%s: Only one process change at a time!", p_cellular_app_pingclient_trace)
  }
...
{
  bool result = false; /* false: distant change not done, true: distant change done */
...
void cellular_app_pingclient_init(void)
{
  ...
  cellular_app_pingclient.queue_id = rtosalMessageQueueNew((const rtosal_char_t *)"CELLAPPIOT_QUE_PING_CMD",
