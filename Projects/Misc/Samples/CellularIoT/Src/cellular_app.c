...
/* Callback called when IP info changed: if (IP !=0U) then data is ready */
...
/* Send to all applications a message */
...
static void cellular_app_propagate_info(ca_event_type_t event_type)
{
  ...
  cellular_app_msg_id_t   msg_id   = CELLULAR_APP_MODEM_CHANGE_ID;
...
bool cellular_app_performance_start(cellular_app_type_t type, uint8_t iter_nb)
{
  ...
  if (result == true)
  {
    ...
    {
      cellular_app_msg_type_t msg_type = CELLULAR_APP_PROCESS_MSG;
      cellular_app_msg_id_t   msg_id   = CELLULAR_APP_PERFORMANCE_ID;
