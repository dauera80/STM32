...
{
  bool exit = false; /* false: continue the send/receive process - true: process goes until its end */
  uint8_t retry;     /* retry number during each step of send/receive process
                      * if equal to 100 exit of send/receive process with error
                      * reset to 0 when a step of send/receive is ok */
  uint8_t i;
  uint16_t plen;
  int32_t res;
  uint8_t ack[2]; /* array to send intermediate command */
      ...
      case NLDC_SENDACK:
        ...
        if (NDLC_SEND_RECEIVE_PHY(p_dev->handlePtr, 2, ack, indata) == 2)
        {
          if (indata[0] == DUMMY_BYTE)
          {
            ...
            /* retry = 0U; */ /* process end with success */
            exit = true;
          }
          else
          {
  ...
  /* Does process goes until the end without issue ? */
  if (exit == true)
  {
    res = 0;
  }
